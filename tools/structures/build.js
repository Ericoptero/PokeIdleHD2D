...
class Mesh {
  ...
  toObj(name, materials) {
    ...
    for (const t of this.vt) out.push(`vt ${t[0].toFixed(5)} ${t[1].toFixed(5)}`);
  ...
  const win = (x, y, z, w, h, axis) => {
    if (axis === 'y') {
      m.quad([[x, y, z], [x + w, y, z], [x + w, y, z + h], [x, y, z + h]], 'window',
        { u0: 0, v0: 0, u1: 1, v1: 0.5, expect: [0, 1, 0] });
    } else {
      ...
      m.quad([[x, y, z], [x, y + w, z], [x, y + w, z + h], [x, y, z + h]], 'window',
        { u0: 0, v0: 0, u1: 1, v1: 0.5, expect: [x > 1 ? 1 : -1, 0, 0] });
    }
  };
...
function pokeMart() {
  ...
  m.quad([[0.6, front, PLINTH + 0.6], [3.2, front, PLINTH + 0.6],
    ...
    { u0: 0, v0: 0, u1: 1.6, v1: 0.5, expect: [0, 1, 0] });
