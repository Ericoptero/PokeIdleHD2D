# Decisions #1–#70 — ARCHIVE, FROZEN

**Closed at #70 (2026-09-09). Nothing is appended here; new decisions start at #71 in
...
process that no longer runs. Both live on in `docs/STATUS-ARCHIVE.json`; neither exists in
`docs/STATUS.json`, and there is no integrator to route a request to.

**It is history, not contract.** Where this file and `ARCHITECTURE.md` disagree about what
the code does today, ARCHITECTURE wins — and if neither matches `src/`, the code wins and
both are wrong.
