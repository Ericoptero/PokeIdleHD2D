...
Windows (slice 016, DECISIONS #85): `windowFrame(g, opts)` takes a required `windowId`, drags by its own title bar (`'window-drag'` region) and resizes by an 8×8 grip in its bottom-right corner (`'window-resize'`), both `drag` regions on the primitives above; `src/ui/window.js` is the pure clamp/default-size math (`clampMove`, `clampResize`, `defaultBox`, `minSizeFor`), and `panels/common.js` holds the stateful per-`windowId` `Map` (`windowGeometry`) and the live-drag reconciliation (`reconcileDrag`, fed the held gesture once a frame by `index.js`'s `setActiveDrag`) — the same `gesture.js`/`screen.js` split, and the same `scrollMemory`-shaped Map, applied to a window's box instead of a scroll offset. Only a window a player has actually dragged or resized gets an entry; every other panel keeps opening at `fit()`'s authored default, centred. `full` (five panels: `shop`, `boxes`, `dex`, `automation`, `party`) is a sizing hint only now — a bigger default `windowFrame` size — and no longer stands the wallet, the clock, the party bar or the button strip down; only `dialogue`'s `hidesHud` does that. `uiScale` (`core/config.js` `DEFAULTS`, 1 or 2) is a config key, not a save field: `screen.js`'s `resize()` divides the renderer's own internal buffer size by it before sizing the UI canvas alone, so the HUD gets bigger with no change to the world's own pixel grid (DECISIONS #60).
