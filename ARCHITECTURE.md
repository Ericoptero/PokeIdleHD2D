...
ladder is a rule, not a check. `settleFrames` is parsed and read by nothing. `actionSteps`
(DECISIONS #89, replacing `turnSteps`) **is** read, by `encounter`'s own `init`.
...
| `player:interact` | ui | `{ cx, cz, dir, facing: { cx, cz }, tags }` — the trainer's cell, facing and the faced cell's tags (`Z`/`Space`, `ui/input.js`) | pokecenter |
| `scene:entered` | travel | `{ sceneId, mapId, biome, formation }` | offline |
| `battle:started` | encounter | `{ index, ally, wild, level, moves }` | — |
| `battle:strike` | encounter (not under `config.showcase`) | `{ index, turn, attacker, attackerSpecies, target, targetSpecies, move, name, struggle, damage, hits, effectiveness, crit, miss, immune, targetHp, targetMaxHp, status, fainted, cause, item, use, type, shape }` — one per beat, `config.actionSteps` apart, never two in the same tick (DECISIONS #89); `type`/`shape` are the move's element and delivery, `null` for a strike with no move | ui |
...
| `slot:respawned` | hunts | `{ biome, slot, species, shiny, level }` | — |
| `ui:toast` | automation, collection, economy, encounter, idle, pokemon, travel, ui | `{ text, kind }` | ui |
...
### 5.6 `encounter` — `needs: ['pokemon', 'terrain', 'economy']` · `showcaseNeeds: ['simulation', 'collection', 'battle']` · hooks `init, tick, lateFrame, dispose, showcase` · selftest **yes**
API: `tablesFor(biome, tod)` → weight-expanded `string[]`; `rollAt(index, { biome, tod, band, rate })` → `{ index, species, level, shiny, ivs, ivTotal, catchRate, … }` (addressed by index, never a continued stream); `catchRollAt(index, turn)`; `slotsNear(cx, cz)` → slot | null (Chebyshev ≤ `config.slotEngageTiles` 1; null with no conscious member, and that refusal `log.warn`s and toasts once per streak — DECISIONS #81); `engage(slot)` (`hunts.takeSlot`, next index, `automation.duel().chooseLead` → `pokemon.setLead`, `begin`); `begin(enc)` (opens `battle.stepper`, `sim.pause(true)`, emits `battle:started`, `encounter:started`); `attempt(ball)` → boolean (one throw, only once `active().battle.win === true`; `economy.throwBall` then the catch coin; emits `catch:succeeded`/`catch:failed`, then `resolve()`); `flee()`; `autoResolve(enc, lead)`; `active()`, `last()`, `transcript()`, `scene()` → `{ stage, step, shake, turns, fighting, at, headLift }`, `ready()`, `refit()`; `cancel()` (no `encounter:resolved`); `advance(n)`, `advanceToStage(stage, frac)`, `setProgress({ steps, encounters })`, `progress()`, `seed()`; `setBall(id)`, `ball()`, `bestBall(enc)`, `oddsFor(id, enc, turn)`, `ballContext(enc, turn)`; `dropsFor(index, { species, biome, catchRate, level, shiny })` → `[{ id, n }]` (pure, index-addressed), `dropTableFor(species, biome)`; `tables()`, `rows(biome, tod)`, `band()`, `shinyRate()`; `armed()` (only at `/` and in this module's own showcase), `freeze(on)`, `frozen()`; `pure()` → `{ rollAt, resolve, dropAt }` (injected into `idle`/`offline` state); `stageStrike(opts)`, `strikes()`; `THROWS_PER_FAINT` (1), `WIPE_PENALTY` (0.10); `saveState()` → `{ v: 1, steps, encounters, ball }`, `loadState(v)`; `selfTest()`; `dispose()`.
Events: emits `battle:started`, `battle:strike`, `battle:ended`, `encounter:started`, `encounter:resolved`, `catch:succeeded`, `catch:failed`, `drop:collected`, `party:wiped`, `ui:toast` (a fainted-party nag, an empty-ball warning, a drop found, a wipe — **never** on an encounter starting: the wild was already on the map and there is nothing to announce, DECISIONS #87) · listens `player:enteredTile` (armed, not frozen, nothing active → `slotsNear` → `engage`), `world:loaded` (clears the table memo). Showcase: `meet` (default and anything unmatched), `vfx-contact`, `vfx-projectile`, `vfx-field`, `walk`, `approach`, `throw`, `shake`, `caught`, `escaped`, `shiny`, `table`, `night`, `balls`. Undeclared: `simulation`, `battle`, `hunts`, `automation`, `collection`, `environment` (all isLive — a quarantined `battle` costs the game its combat, not its encounters).
**A fight opens on contact, not on a reveal** (DECISIONS #87): the creature `hunts` hands over was already standing on its cell, `begin()` draws it there with no rustle, hop or "!" balloon, and the first exchange steps `T.OPEN` (10 sim steps) later. `battle.stepper` is asked for one turn at a time and its strikes are drained **one per beat, never two in the same tick** (DECISIONS #89) — `config.actionSteps` (18) apart, longer for an item or a revive (`beats.js`'s `planBeats`, pure and index-addressed like every other roll here); an unattended fight resolves itself with no ball spent. `resolve()` credits money (`economy.add('money', …, 'battle')` — STATUS `watched-win-mints-money`), grants exp through `pokemon.grantPartyExp`, hands loot to `economy.give(id, n, 'drop')`, and on a wipe takes `WIPE_PENALTY` of the wallet, `reviveAll`s and emits `party:wiped` (`travel` hops) — **every** wipe, not only the first (DECISIONS #81). The duel's `between` hook debits items through `economy.take`. **Move VFX is three shaders** (`encounter/vfx/{elements,particles,beams,ground,play}.js`, DECISIONS #91 — a deliberate, named exception to this project's pixel-art-only doctrine for this system alone): `vfx/elements.js` is pure data — `shapeOf(move)` (contact/projectile/field, unchanged from the system it replaces), `profileFor(type)` → `{motion, role, intensity}` (eighteen types, five motions × six particle roles), and `BEATS[shape]` — a four-beat `charge → deliver → impact → resolve` timeline covering `[0,1]`. `vfx/play.js`'s `makeStrikeVfx(THREE, ctx)` keeps the exact `play({shape,type,from,to,crit,effectiveness})`/`phase(p)`/`hide()`/`playing()`/`dispose()` contract the old system offered, reads `battle.typeColour(type)` for `core`/`edge` (no more copy of that table here), and drives one shared particle field (`particles.js`, a single `ShaderMaterial`/`BufferGeometry` of 128 quads, camera-facing by construction — no `refit()`), one ribbon (`beams.js`, a projectile's trail or a contact's arc slash, its width kept perpendicular to its own on-screen curve rather than a fixed axis), and one ground ring (`ground.js`, `depthTest:true`, a field move's rune or any shape's impact shockwave — doubled on a super-effective hit, a crit adds one bright white-flash beat). `encounter/index.js`'s own changes are an import swap, `scene.vfx` gaining `crit`/`effectiveness`, and its render-rate hook moving from `frame` to `lateFrame` (`ball.js`'s own sprite still needs a camera-facing refit there; the new VFX no longer does).
...
### 5.12 `ui` — `needs: []` · `showcaseNeeds: ['city', 'economy', 'collection', 'offline', 'idle', 'automation', 'hunts', 'travel', 'battle', 'encounter']` · hooks `init, tick, frame, lateFrame, dispose, showcase` · selftest **yes**
API: `toast(text, kind)`; `open(id, opts)` (one panel at a time; ids `menu`, `travel`, `offline`, `shop`, `boxes`, `dex`, `automation`, `party`, `inventory`, `trainer`, `battle`, `dialogue`); `close()`; `isOpen()`; `openPanel()` → id | null; `say(text, opts)`; `showReport()`; `snapshot()`; `metrics()` → `{ width, height, drawCalls }`; `project(x, y, z)` → `{ x, y }` | null; `evolution.play(spec)`, `evolution.freeze(spec, t)`, `evolution.close()`, `evolution.TOTAL`, `evolution.BEATS`; `input.press(dir)`, `input.release(dir)`, `input.setTouch(on)`; `saveState()` → `{ v: 1, windows }` (per-panel window geometry, slice 016), `loadState(v)`; `selfTest()`; `dispose()`.
Events: emits `ui:toast`, `player:interact` (`Z`/`Space` outside a panel, `input.js`: the trainer's cell, facing and the faced cell's tags via `terrain.tagsAt`, off `simulation.player()`) · listens `ui:toast`, `battle:strike`, `encounter:started` (opens the battle card, never over a panel the player opened, never in another module's showcase), `encounter:resolved` (closes it), `economy:changed`, `collection:added`, `party:leadChanged`, `pokemon:evolved` (the DOM cutscene; never under `config.showcase`), `tod:changed`, `offline:applied` (opens the away card). Showcase: `default`/`hud`/`font`, `menu`, `shop`, `boxes`, `dex`, `party`, `moves`, `travel`, `battle`, `evolution`, `evolution-burst`, `evolution-reveal`, `evolve`, `offline`, `toasts`, `dialogue`, `input`. Undeclared: `pokemon`, `economy`, `environment`, `simulation`, `terrain`, `offline`, `encounter`, `travel`, `automation`, `idle`, `battle`, `collection`, `hunts` (all isLive).
One 2-D canvas at the renderer's internal resolution, zero draw calls; the evolution cutscene is the one DOM/CSS piece. Keys (`input.js`): arrows/WASD move when `simulation.formation().input`; KeyT travel, KeyP party, KeyB shop, KeyC boxes, KeyI inventory, KeyR trainer, KeyM menu, KeyU automation, Digit1–8; Escape/X close. `PANEL_IDS` lives in `input.js` and `selfTest()` checks it against the live `PANELS`. The battle card throws (Z/Enter/Space) and runs (R) only once `win === true`; `screen.regions()` publishes the hit boxes of the last paint, each carrying `tag`, `box` and now (DECISIONS #84) `swallow`/`drag`/`drop`/`scroll` booleans — a control's *mode*, not merely that it exists; a panel's own `draw(g)` also returns its box directly, so `plates.js` (below) can keep a nameplate off whichever panel is open.
The inventory panel (`panels/inventory.js`, slice 018) is a grid over `economy.bag()`/`stash()` — every held item, tab-switched Bag/Stash and filtered by category, with the sell-lock toggle (`economy.setSellLock`) as its one write; `economy` has no bus event for a bare item-count change (`economy:changed` is currency-only), so the panel marks itself dirty off `economy.onChange(fn)` instead. Category marks are `theme.js`'s `itemMark(g, x, y, category, tier)` — a placeholder bitmap per category (seven today), tinted by tier off the dark-recess ramp; an `open` `docs/STATUS.json` entry tracks real art. The pointer layer (`screen.js`): `g.hit(box, {on, drag, drop, scroll, swallow}, tag)` alongside the original `g.hit(box, on, tag)`; a gesture picked up with `drag` survives the `paint()` reset that clears `regions` every frame (`src/ui/gesture.js`'s pure reducer — `screen.drag()` reads the live one); `g.clip(x, y, w, h, draw)` clips a draw callback to a rectangle. `common.js`'s `list()` gained a wheel path with no change to its call sites (a per-`tag` scroll memory reconciles a caller-owned `top` with a wheel delta); `scrollArea(g, box, opts)` is the same primitive for non-uniform content — `panels/trainer.js` (slice 019, below) is its first caller.
Windows (slice 016, DECISIONS #85): `windowFrame(g, opts)` takes a required `windowId`, drags by its own title bar (`'window-drag'` region) and resizes by an 8×8 grip in its bottom-right corner (`'window-resize'`), both `drag` regions on the primitives above; `src/ui/window.js` is the pure clamp/default-size math (`clampMove`, `clampResize`, `defaultBox`, `minSizeFor`), and `panels/common.js` holds the stateful per-`windowId` `Map` (`windowGeometry`) and the live-drag reconciliation (`reconcileDrag`, fed the held gesture once a frame by `index.js`'s `setActiveDrag`) — the same `gesture.js`/`screen.js` split, and the same `scrollMemory`-shaped Map, applied to a window's box instead of a scroll offset. Only a window a player has actually dragged or resized gets an entry; every other panel keeps opening at `fit()`'s authored default, centred. `full` (six panels: `shop`, `boxes`, `dex`, `automation`, `party`, `inventory`) is inert data — nothing reads it any more, for sizing or anything else; it no longer stands the wallet, the clock, the party bar or the button strip down, and is kept on the descriptor only as a record of which panels used to (and for `battle.js`'s own header comment, which contrasts its `full: false`). Only `dialogue`'s `hidesHud` still stands the bars down. `uiScale` (`core/config.js` `DEFAULTS`, 1 or 2) is a config key, not a save field: `screen.js`'s `resize()` divides the renderer's own internal buffer size by it before sizing the UI canvas alone, so the HUD gets bigger with no change to the world's own pixel grid (DECISIONS #60). `app.hudReserved()` reports how many pixels at the top and bottom of the buffer the wallet/clock and the party bar/button strip already occupy *this frame* (`{top, bottom}` — zero when `hidesHud`); every `windowFrame` call passes it through as `opts.reserved`, and `window.js`'s `clampToSafeArea(box, buffer, margin, reserved, min)` is the final clamp that keeps a window's box from opening, defaulting, or being dragged/resized onto them — shrinking a window's height below its own `min` when the safe area itself is smaller than that, since avoiding the bars wins over honouring a size floor a player can still enlarge (post-review fix to slice 016, folded into DECISIONS #85: a `full` panel at `uiScale:2` previously painted straight over the bars it was supposed to leave visible).
The trainer panel (`panels/trainer.js`, slice 019) is a pure consumer of `economy` (`trainer()`, `progress()`, `stats()`, `currencies()`, `multipliers()`, `buffs()`, `bag()`, `upgrades()`), `collection.completion()` and `pokemon.party()` — no new economy API, no new event. It is the first panel built on `scrollArea` rather than a fixed-height window: `trainerModel(app)` (exported, the `travel.js`/`rows()` and `inventory.js`/`filterRows` precedent) builds the five stacked sections — Progress, Bonuses, Upgrades, Dex, Party — above the portrait/level/wins bar and the four currencies, and `layout(model)` turns that model into block heights with no `g` at all, so the content's total height is asserted on directly in `trainer.test.js` rather than read off a screenshot. The portrait is `/assets/trainer/hero.png` always: `simulation`'s own `trainerWho` (the party's on-screen avatar) never varies at runtime today, so there is no live "which trainer" to read, and the frame crop (32×32, south row's frame 11) is a small local constant rather than an import of `pokemon/sprites.js` (rule 2 forbids the deep import).
**Nameplates** (`plates.js`, `?scene=` or `?showcase=ui` only — suppressed like everything but the wallet/clock/toasts in another module's showcase): name/level/HP for the trainer, the party's lead and every wild in the current hunt (`hunts.slots()` for a wandering one, `encounter.active()`/`scene()` for the one being fought — its live duel HP, the same `panels/battle.js` already reads); name only for every other `simulation.npcs()` entry (a city local, Nurse Joy — `display`, then a Pokémon NPC's own species name, then its slug, in that order). Clamped off the party bar, the button strip and the battle card's own box; capped at 14 tiles (Chebyshev) from the trainer so a wide framing does not print the whole map's roster at once, and at `MAX_PLATES` (48) absolute; two that still collide are stacked, and one that still cannot fit is dropped for that frame rather than drawn on top of another. Painted in `lateFrame`, after the camera rig updates (`core/registry.js`, `src/main.js`) — the same reason `pokemon/field.js` poses sprites there and not in `frame`: a plate projected against the previous frame's camera trails a moving sprite by one frame of motion. Marks the screen dirty whenever there is a plate to draw, since a plate tracks something that can move every rendered frame and not merely every sim tick.

**Balloons and floaters** (`callout.js`, `floaters.js`, DECISIONS #90): a `battle:strike` puts up one balloon per side (replacing, not stacking) with a small downward tail pointing at the true projected anchor even when the box itself leans clear of it, and one floater over the *target* — never the attacker, and never the trainer even when the ally is the attacker (a hit lands on the Pokémon, not on the trainer standing behind it, so a floater reads `simulation.follower()` where a callout for the same side reads `simulation.player()`). Only the move's own name inside the balloon is coloured by its type (`battle.typeColour(type).ink`); the balloon's paper stays the same neutral panel every other one here uses. A floater is `-N` for damage (`C.roofShadow` on a super-effective hit, `C.shadowInk` on a resisted one, doubled in size via `screen.js`'s `textScaled` on a crit), `MISS`/`IMMUNE` for either, or the inflicted status's abbreviation (`panels/battle.js`'s own `STATUS_NAME`) for a pure status hit — never more than one per strike. Both are lifted clear of the plate at the same lift `plates.js` uses for that speaker, plus a shared clearance constant, so neither one prints across the name/level/HP row now living at the same anchor.
...
Events: emits none · listens `world:unloaded` (teardown). Showcase: `default`, `plaza`, `centre`, `pokecenter`, `mart`, `high-street`, `south-gate`, `pond`, `wood-yard`, `garden`, any `cx,cz`, any marker (`pokecenter-door`, `mart-door`, …). Undeclared: `simulation`, `pokemon` (isLive), `tiles` (unguarded). Tilesets: `bw2-adastra`, `pt-overworld-7` (plaza paving), `structures`, `props`. `env.lamps.clear()` in `enter()` wipes every module's lamps. **`enter()` no longer heals the party** — the free, unconditional heal DECISIONS #81 gave this module is deleted (DECISIONS #83); the cure is now Nurse Joy, inside `pokecenter` (§5.18).
...
Biomes (`biomes/*.js`): `meadow` (requiredLevel 0), `forest` (5), `coast` (12), `cave` (20); formation `{ head: 'pokemon', input: false, autopilot: 'route', strict: true }`. API: `list()` → `[{ id, name, preset, tileset, w, h, presets, formation, requiredLevel, loop | null, slots: count }]`; `biome(id)`; `current()`; `enter(id)` async → handle (environment preset + weather, `terrain.load('hunt-<id>')`, extras worlds, lamps, the wild cast, `setFormation` with the route rotated by `config.followerGapTiles`, `teleport(loop.start)`, `audit()`); `preset(name)` (a biome preset or `cx,cz`); `loop(id)` → `{ start, route, w, h, corners, length, cells }`; `slots(id)` → `[{ k, cx, cz, dir, from, step, approach, occupied, species, display, shiny, level, npcId }]` (`level` is rolled when the creature walks onto the slot, from its own seeded `hunts/level/<biome>/<k>/<gen>` stream against `encounter.band()` — not at engagement, so the number a plate shows before a fight is the number it fights at, DECISIONS #87); `takeSlot(k)` → `{ species, shiny, level, cx, cz, dir, k, npcId }` | null (hands the creature over **without** removing its NPC — `cx,cz,dir` are read live off `simulation.npcs()`, i.e. where the tether has actually drifted it to, not the slot's authored anchor; the caller retires `npcId` once its own actor exists; refills after `respawnSeconds` 26 s from `encounter.tablesFor` with a 1/512 shiny roll and the same level roll); `wild()`; `stats(id)`; `markers()`; `audit(id)` → `{ ok, checked, fails }`; `respawnSeconds`; named export `BIOMES`.
Events: emits `hunt:lap` (every full lap each member below full heals `config.lapHealFraction` 0.34 of maxHp — a fainted one is revived to it with its status cleared, a conscious one is topped up and keeps its status (DECISIONS #81)), `slot:respawned` (`{ biome, slot, species, shiny, level }`) · listens `player:enteredTile` (a detour onto an occupied slot: `holdNpc` then `sim.detour([step, back])`, once per lap per slot), `world:unloaded`. Showcase: `forest` (default and unknown, at `warn`), `meadow`, `cave`, `coast`. Undeclared: `simulation`, `pokemon` (isLive), `tiles` (unguarded). No save slice: occupancy and refills reset on every `enter()`.
...
Events: emits `scene:entered`, `ui:toast` · listens `party:wiped` (a microtask later: `go('pokecenter')` — landing on `pokecenter.enter()`'s own `SPAWN`, no extra teleport — falling back, only if that returns `false` (a quarantined `pokecenter`), to `go('demo-city')` then `teleport` to the `pokecenter-door` marker facing north; skipped under showcase). Showcase: any destination id, else `demo-city`; then opens the travel panel. Undeclared: `city`, `hunts`, `pokecenter`, `economy`, `encounter`, `simulation`, `ui`, `offline` (all isLive; the slice is self-registered at order 45 because `travel` inits after `offline`). The door-to-door hop in and out of the Center is `pokecenter`'s own `player:enteredTile` listener (§5.18), not this module's.
...
API: `enter()` async → terrain handle (biome preset `interior`, weather clear, `terrain.load('pokecenter', { w: 13, h: 10, tileset: 'pt-house-indoor', biome: 'city' })` — `biome: 'city'` on purpose: `'interior'` is not in `encounter`'s `BIOMES` and would fall back to the **meadow** spawn table, where `'city'` already resolves to `TABLES.city = []` — dressing, `setFormation` + `placePlayer` at the room's own spawn facing the counter, then spawns Nurse Joy — `simulation.spawnNpc({ trainer: 'heroine', solid: true, … })` at `NURSE`, behind the counter — after `dress()` resolves); `preset(name)` → boolean (a `PRESETS` key, a `cx,cz` literal or a draft marker; teleports the trainer); `presets()`; `formation()` → `{ head: 'trainer', input: true, autopilot: 'none' }`; `markers()`; `marker(name)`; `stats()`; `saveState()` → `{ v: 1, lastHealMs }`, `loadState(v)` (a non-finite/missing `lastHealMs` reads as "never healed" — no self-registration needed, §10: `pokecenter` inits before `offline` in the Kahn order, so `offline.init()`'s `discoverProviders` finds this native pair on its own, unlike `travel`); `dispose()`.
Events: listens `world:unloaded` (teardown — also despawns Nurse Joy), `player:enteredTile` — the door, both ways: `tags.includes('door:pokecenter')` while `travel.current().id === 'demo-city'` calls `travel.go('pokecenter')`; the room's own exit tag while `current().id === 'pokecenter'` calls `travel.go('demo-city')` and re-teleports onto the city's `pokecenter-door` marker, facing south (a wiped party's teleport, §5.16, faces north instead — that is a fainted party arriving to be healed, this is a player who already saw the counter) — `player:interact` (§4, `ui`): when the faced cell carries the `counter` tag (all 3 counter cells, `map.js`) and the player is in this room, opens a dialogue with Nurse Joy (`ui.say(…, {speaker:'Nurse Joy'})`). Off cooldown (`heal.js`'s `remainingCooldownMs(lastHealMs, ctx.clock.wallMs())`, `HEAL_COOLDOWN_MS` 60 000 ms) the cure runs unconditionally — `pokemon.reviveAll()` (the `revive: true` allowlist's existing caller #1, `pokemon/instance.js:230-266`) plus `restorePp(instanceId, {moveId, amount:'full'})` on every party member's every move slot, the actual difference from every other recovery path — and stamps `lastHealMs`; on cooldown, nothing is touched. Showcase: `default`, `counter`. Undeclared: `simulation`, `travel`, `pokemon`, `ui` (isLive), `tiles` (unguarded). Tilesets: `pt-house-indoor`, `hgss-newbark-houses`, `bw2-adastra`. `env.lamps.clear()` in `enter()` wipes every module's lamps. Three walls only — south is collision without geometry, so the fixed camera can see into the room; DECISIONS #82 is why the west and east walls barely read as solid either way. **`city.enter()` no longer heals** — that free, unconditional net (DECISIONS #81's third) is deleted; Nurse Joy replaces it (DECISIONS #83). A party wipe still arrives already healed (`encounter.wipe()`'s own `reviveAll()`, untouched), just in this room now, and never arms this module's cooldown.
...
- Tiles come from Pokémon DS Map Studio tilesets; `bw2-adastra` is the default, loaded at `tiles` init. Other sets are
  ...
  **One named exception:** a strike's own effect (§5.6, `encounter/vfx/*.js`) is coloured primitives and procedural
  shaders, not pixel art painted to a canvas — a deliberate, user-requested departure from this rule for that one
  system alone (DECISIONS #91). Nothing else in the game is exempted by this entry.
...
| 50 | `battle`, `collection`, `encounter`, `pokecenter` | native: `{ v: 1, priority }`, `{ v: 1, ordinal, overflow, dex, boxes, stored }`, `{ v: 1, steps, encounters, ball }`, `{ v: 1, lastHealMs }` — the cure's cooldown only; the map itself is rebuilt from its seed and Nurse Joy is a fixed NPC |
| 55 | `ui` | native `{ v: 1, windows }` (per-`windowId` `{x,y,w,h}`, only ids a player has dragged or resized), self-registered (inits after `offline`, DECISIONS #85) |

`hunts` has no slice (slot occupancy is runtime-only). Under `config.showcase` the store is read-only (§6).
