/**
 * The balloon's per-type colour and the damage floater (DECISIONS #87), proven against a real
...
test('a real strike colours the move by its type, and drops a damage number on the target', async ({ page }) => {
  ...
  expect(hit.payload.type, 'the strike carries its element (DECISIONS #86)').toEqual(expect.any(String));
